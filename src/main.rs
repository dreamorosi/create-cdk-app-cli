...
fn create_directories(paths: &ProjectPaths) -> Result<(), String> {
    ...
    if let Err(e) = std::fs::create_dir_all(&paths.lib_dir) {
        return Err(format!(
            "Could not create lib directory '{}': {}",
            paths.lib_dir, e
        ));
    }
    ...
    if let Err(e) = std::fs::create_dir_all(&paths.events_dir) {
        return Err(format!(
            "Could not create events directory '{}': {}",
            paths.events_dir, e
        ));
    }
    Ok(())
...
fn write_project_files(
    ...
    if let Err(e) = std::fs::write(
        format!("{}/package.json", paths.base_dir),
        &files.package_json,
    ) {
        return Err(format!("Could not write package.json: {}", e));
    }
    if let Err(e) = std::fs::write(
        format!("{}/package-lock.json", paths.base_dir),
        &files.package_lock_json,
    ) {
        return Err(format!("Could not write package-lock.json: {}", e));
    }
    ...
    if let Err(e) = std::fs::write(
        format!("{}/vitest.config.ts", paths.base_dir),
        &files.vitest_config,
    ) {
        return Err(format!("Could not write vitest.config.ts: {}", e));
    }
    if let Err(e) = std::fs::write(
        format!("{}/{}.ts", paths.bin_dir, app_names.lower),
        &files.bin_file,
    ) {
        return Err(format!("Could not write bin file: {}", e));
    }
    if let Err(e) = std::fs::write(
        format!("{}/index.test.ts", paths.test_dir),
        &files.test_file,
    ) {
    ...
    if let Err(e) = std::fs::write(
        format!("{}/{}-stack.ts", paths.lib_dir, app_names.lower),
        &files.lib_file,
    ) {
    ...
    if let Err(e) = std::fs::write(
        format!("{}/payload.json", paths.events_dir),
        &files.events_file,
    ) {
        return Err(format!("Could not write payload.json: {}", e));
    }
    if let Err(e) = std::fs::write(
        format!("{}/cdk.context.json", paths.base_dir),
        &files.cdk_context,
    ) {
    ...
    match output {
        Ok(result) => {
            if !result.status.success() {
                spinner.fail("npm ci failed!");
                let mut error_msg =
                    format!("npm ci failed with exit code: {:?}", result.status.code());
                ...
                error_msg
                    .push_str("\nYou can try running 'npm ci' manually in the project directory.");
                return Err(error_msg);
            }
            spinner.success("New CDK project created successfully!");
        }
        Err(e) => {
            spinner.fail("Failed to run npm ci!");
            return Err(format!(
                "Could not execute npm: {}. Make sure npm is installed and available in your PATH.",
                e
            ));
        }
    }
...
fn main() {
    ...
    let files = match load_template_files(&app_names) {
        Ok(files) => files,
        Err(e) => {
